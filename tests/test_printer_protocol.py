"""Pin down the exact byte sequences the LuckPrinter driver puts on the wire.
...
connection. The point isn't to test the classes in the abstract - it's to
snapshot the actual protocol bytes (wake sequence, density command, GS v 0
header, feed/end commands) so a driver/session refactor can be checked
against real, previously-working output instead of trusting it by
inspection. These assertions are unchanged from before the PrinterSession/
PrinterDriver split - same bytes, different wiring - which is exactly what
proved that refactor didn't alter behavior.
"""
...
def no_real_delays(monkeypatch):
    # PrinterSession deliberately sleeps between commands to match the
...
def make_session(device, **kwargs):
    return PrinterSession(device, LuckPrinterDriver(), **kwargs)
...
def test_build_gs_v_0_header_layout():
    command = build_gs_v_0(b"\x01\x02", width_bytes=1, height_px=2, mode=0)
...
def test_print_image_command_sequence_with_density():
    device = FakeDevice()
    session = make_session(device, width=8, density=1)
...
def test_print_image_command_sequence_without_density():
    device = FakeDevice()
    session = make_session(device, width=8, density=None)
...
def test_print_image_chunks_large_payloads():
    device = FakeDevice()
    session = make_session(device, width=384)
    img = Image.new("L", (384, 200), color=0)  # all-black -> big bitmap

    run(session.print_image(img, dither=False))

    bitmap, width_bytes, height_px = image_to_bitmap(img, 384)
    expected_image_command = build_gs_v_0(bitmap, width_bytes, height_px)
    # Everything after the fixed 2-command preamble (enable + wake) is the
    # image command, split into chunk-sized pieces in order.
...
def test_print_end_feed_dots_is_masked_to_a_byte():
    ...
    run(session.print_end(feed_dots=300))  # out of single-byte range
